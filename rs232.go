...
func Open(cfg Config) (*Rs232, error) {
    fd, err := syscall.Open(cfg.Name, syscall.O_NOCTTY|syscall.O_CLOEXEC|syscall.O_RDWR, 0666)
    if err != nil {
        return nil, &os.PathError{"open", cfg.Name, err}
    }
    s := Rs232{config: cfg, fd: fd}
    if err = termios.Tcgetattr(uintptr(s.fd), &s.orig); err != nil {
        return nil, err
    }
...
func (s *Rs232) Configure(cfg Config) error {
    ...
    if err := termios.Tcgetattr(uintptr(s.fd), &attr); err != nil {
        return err
    }

    // Select raw mode
    attr.Iflag &^= syscall.BRKINT | syscall.ICRNL | syscall.INPCK | syscall.ISTRIP | syscall.IXON
    attr.Lflag &^= syscall.ECHO | syscall.ICANON | syscall.ECHOE | syscall.IEXTEN | syscall.ISIG
    attr.Oflag &^= syscall.OPOST

    attr.Cflag = syscall.CREAD | syscall.CLOCAL
    ...
    if s.config.ReadTimeout > 0 {
        ...
        attr.Cc[syscall.VTIME] = s.config.ReadTimeout
    } else {
    ...
    s.config = cfg
    return termios.Tcsetattr(uintptr(s.fd), termios.TCSANOW, (*syscall.Termios)(&attr))
}

// Close closes the communication between two rs232 device.
